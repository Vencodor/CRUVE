				...
				<ul>
					...
					<li class="menu--item">
						<a href="live_streams.jsp" class="menu--link" title="Live Streams">
							...
							<span class="menu--label">판매자</span>
						</a>
					</li>
					<li class="menu--item">
				...
				<ul>
					...
					<li class="menu--item">
						<a href="all_instructor.jsp" class="menu--link" title="Browse Instructors">
						  ...
						  <span class="menu--label">즐겨찾기 등록</span>
						</a>
					</li>
				</ul>
			</div>
							...
							<div class="explore_search">
								<div class="ui search focus">
									<div class="ui left icon input swdh11">
										<input class="prompt srch_explore" type="text" id="search" placeholder="이름을 검색하세요...">
...
if(users !=null) {
	...
	if(users.size()!=pages) loaderEnd = true;
...
%>
								<div class="col-xl-3 col-lg-4 col-md-6">
									<div class="fcrse_1 mt-20">
										<div class="tutor_img">
											<a href="profile_view.jsp?ID=<%=user.getUserName()%>"><img src="./ProfileImages/<%=user.getProfileImage()%>" alt=""></a>												
										</div>
...
%>
	...
	$(function() {
		if (String(<%=loaderEnd%>) == "false") {
			$(window).scroll(
				function() {
					if ($(this).scrollTop() >= Math.ceil($('.scroll-loader').offset().top) - 630) {
						page = Number(<%=pages%>) + 12;
						$('#usersList').load(location.href+'?page='+page+' #usersListHTML');
					} else {
					}
				}
			)
	...
	$(function() {
    	$('#search').keydown(function(e) {
        	if (e.keyCode == 13) {
        		$('#usersList').load(location.href+'?search='+$('#search').val()+' #usersListHTML');
            }
    	});
    });
