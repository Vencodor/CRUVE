...
<html lang="en" <%if(session.getAttribute("mode")!=null){if((Boolean)session.getAttribute("mode")==true) {%>class=" night-mode"<%}}%>>
	...
	<head>
		...
		<title>Cursus - Report History </title>
		...
		<div class="left_section menu_left" id="js-menu" >
			...
			<div class="left_section pt-2">
				<ul>
					...
					<li class="menu--item">
						<a href="report_history.jsp" class="menu--link active" title="Report History">
						...
						<div class="row">
							<div class="col-lg-12">
								<div class="report_history">
									<h4>적극적인 신고 감사드립니다.</h4>
									<p>CRUVE의 모든 회원은 커뮤니티 가이드 라인을 위반한다고 생각되는 콘텐츠를 신고 버튼을 통해 신고 할 수 있습니다. 신고 된 항목은 자동으로 삭제되지 않습니다, 신고 된 콘텐츠는 다음 지침에 따라 검토됩니다.</p>
									<ul>
										<li><p><a href="#">Community Guidelines</a> 를 위반하는 콘텐츠는 삭제됩니다.</p></li>
										<li><p>모든 청소년에게 적합하지 않은 콘텐츠는 제한 될 수 있습니다.</p></li>
									</ul>
									<a href="#" class="lnk586"></a>
									<span>
		<div class="mb4d25" id="reportField">
			<div class="container" id="reportFieldHTML">			
				<div class="row">
<%
	ArrayList<ReportDTO> list = new ArrayList<ReportDTO>();
	
	list = reportDAO.getList(userID);
	int listSize = 0;
	
	try { listSize = list.size(); } catch(Exception e) { e.printStackTrace(); }
	
	for(int i=0; i<list.size(); i++) {
		ReportDTO a = list.get(i);
		int typeID = 0;
		String type = null;
		if(a.getReviewID()!=0) {
			typeID = a.getReviewID();
			type="리뷰 신고";
		}
		else {
			typeID = a.getWrittenID();
			type="상품 신고";
		}
%>
					<div class="col-lg-6" style="height:130px">
						<div class="fcrse_1">
							<div class="hs_content">
								<a href="detail_view.jsp?ID=<%=aes.encrypt(a.getWrittenID())%>" class="crse-cate"><%=type%> | <%=a.getLastDate()%></a>
								<div class="auth1lnkprce">
									<p class="cr1fot">사유 : <%=a.getDescription()%></p>
									<div class="prce142"><%if(a.isOK()) {%><span style="color:green;">처리완료</span><%} else {%><span style="color:red;">미처리</span>
									<a href="#" onclick="return false;" id="cancelReport" data-id="<%=a.getReportID()%>">
									<%if(!userID.equals("1324qqww")) {%>취소<%} else {%>활성화<%} %> <i class="fas fa-times"></i></a>
									<%}%></div>
								</div>
							</div>
						</div>	
	...
	$(function() {
		$(document).on('click','#cancelReport',function() {
			$.ajax({
	            ...
	            url:'http://localhost:8080/CRUVE/setReportCancel',
	            dataType:'text',
	            ...
	            success: function(data, textStatus) {
	            	if(data.indexOf("success")!=-1) {
	            	    ...
	            	    title: '정상적으로 처리되었습니다.' })
	            	    $('#reportField').load(location.href + ' #reportFieldHTML');
	            	} else {
	            		Toast.fire({ icon: 'success', 
		            	    title: data })
	            	}
